...
def main():
    ...
    decoder_input_dim = hidden_dim + output_dim
    ...
    run_name = f'enc_only,epochs={n_epochs},batch_size={batch_size},hidden_dim={hidden_dim},timestamp={timestamp}'
    ...
    for fold, (train_index, val_index) in enumerate(splits.split(np.arange(len(dataset)))):
        ...
        for epoch in range(n_epochs):
            for i, batch in enumerate(tqdm(train_dataloader, desc=f'fold={fold}, epoch={epoch} progress')):
                ...
                y = model.forward(X)
                loss = loss_fn(y.view(-1, output_dim),
                               t.view(-1, output_dim))
                loss.backward()
                optimizer.step()

...
class Seq2SeqPOSTagger(nn.Module):
    ...
    #     enc_tag_tensor = torch.cat((encoder_output, t), dim = 2).to(self.device)
    ...
    #     for tag_index in range(t.shape[1]):
    #         decoder_output_t = self.decoder(
    #             decoder_input, decoder_hidden_state, decoder_cell_state)
    #         decoder_output[:, tag_index, :] = decoder_output_t.squeeze(1)
    #         decoder_input = enc_tag_tensor[:, tag_index, :].unsqueeze(1)
